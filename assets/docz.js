document.addEventListener('DOMContentLoaded', function () {
    ...
    loadDocument(location.hash.substring(1))
    window.onpopstate = () => loadDocument(location.hash.substring(1))

    menu.addEventListener('click', () => {
        sidebar.classList.toggle('slide')
    })
    fetch('/SIDEBAR.md').then(res => res.text()).then(res => {
        nav.innerHTML = marked.parse(res)
        activeCurrentLink(location.hash.substring(1))
        addLinksClick()
    })
